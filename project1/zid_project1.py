...
def get_tics(pth):
    ...
    with open(pth, 'r') as f:
        for line in f:
            line = line.strip()
            lst = line.split('=')
            tickers_dict[lst[1].strip('"').lower()] = lst[0].strip('"').lower()
    return tickers_dict
...
def read_dat(tic):
    ...
    """
    ticker_path = os.path.join(DATDIR,f'{tic}_prc.dat')
    ticker_list = []
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    if not tickers_lst:
        raise ValueError("tickers_lst is empty")
...
def verify_cols(col_lst=None):
    ...
    for col in col_lst:
        try:
            col_change = COLUMNS.index(col)
        except KeyError:
            raise ValueError(f"{col} is not a valid ticker symbol")
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    # If tickers_lst is None, get all tickers from tic_exchange_dic
    if tickers_lst is None:
        tickers_lst = list(tic_exchange_dic.keys())

    # If col_lst is None, use all columns in COLUMNS
    if col_lst is None:
        col_lst = COLUMNS
    ...
    for ticker in tickers_lst:
        ...
        tic_list = read_dat(ticker)
        lst = []
        for element in tic_list:
            info = line_to_dict(element)
            exact_info = {}
            for col in col_lst:
                exact_info[col]=info[col]
            lst.append(exact_info)
        data_dict['data'] = lst
        total_dict[ticker] = data_dict
    return total_dict
